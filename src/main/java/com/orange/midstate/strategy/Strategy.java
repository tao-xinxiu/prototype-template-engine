...
public abstract class Strategy {
	protected static List<AppProperty> updateOrder = Collections
			.unmodifiableList(Arrays.asList(AppProperty.Path, AppProperty.Env, AppProperty.State,
					AppProperty.Routes, AppProperty.Instances, AppProperty.Name));
	...
	public Set<OverviewApp> nothingUpdated() {
		...
		desiredRelatedApps.remove(
				Util.exludedApps(desiredRelatedApps, instantiatedDesiredApp(desiredRelatedApps)));
	...
	protected Set<OverviewApp> directlyUpdateProperty(AppProperty property) {
		...
		try {
			...
			Method setPropertyMethod = OverviewApp.class.getMethod("set" + property,
					getPropertyMethod.getReturnType());
			setPropertyMethod.invoke(instantiatedDesiredApp(desiredRelatedApps),
					getPropertyMethod.invoke(desiredApp));
