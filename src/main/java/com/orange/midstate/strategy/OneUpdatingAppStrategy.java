...
public abstract class OneUpdatingAppStrategy extends Strategy {
    ...
    public OneUpdatingAppStrategy(Set<OverviewApp> currentApps, OverviewApp desiredApp, SiteDeploymentConfig config) {
	super(currentApps, desiredApp, config,
		app -> desiredApp.getGuid() != null ? desiredApp.getGuid().equals(app.getGuid())
			: Arrays.asList(desiredApp.getName(), desiredApp.getName() + config.getTmpNameSuffix())
